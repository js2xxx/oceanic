...
// When attribute privacy is implemented, `OsStr` should be annotated as
// `#[repr(transparent)]`. Anyway, `OsStr` representation and layout are
// considered implementation details, are not documented and must not be relied
// upon.
pub struct OsStr {
    inner: Slice,
}
...
impl From<OsString> for Box<OsStr> {
    /// Converts an [`OsString`] into a <code>[Box]<[OsStr]></code> without
    /// copying or allocating.
    #[inline]
...
impl From<OsString> for Arc<OsStr> {
    /// Converts an [`OsString`] into an <code>[Arc]<[OsStr]></code> by moving
    /// the [`OsString`] data into a new [`Arc`] buffer.
    #[inline]
...
impl From<OsString> for Rc<OsStr> {
    /// Converts an [`OsString`] into an <code>[Rc]<[OsStr]></code> by moving
    /// the [`OsString`] data into a new [`Rc`] buffer.
    #[inline]
...
macro_rules! c_str {
    ($e:expr) => {{
        ...
        const STR_VALID: bool = $crate::ffi::validate_c_str_constant(STR);
