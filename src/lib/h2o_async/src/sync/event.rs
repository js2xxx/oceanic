//! Edit: Copied from `event-listener`.
//!
//! Notify async tasks or threads.
//!
//! This is a synchronization primitive similar to [eventcounts] invented by
//! Dmitry Vyukov.
//!
//! You can use this crate to turn non-blocking data structures into async or
//! blocking data structures. See a [simple mutex] implementation that exposes
//! an async and a blocking interface for acquiring locks.
//!
...
//!
//! let flag = Arsc::new(AtomicBool::new(false));
//! let event = Arsc::new(Event::new());
//!
...
struct Inner {
    /// The number of notified entries, or `usize::MAX` if all of them have been
    /// notified.
    ///
    ...
    /// A single cached list entry to avoid allocations on the fast path of the
    /// insertion.
...
///
/// Listeners can be registered using [`Event::listen()`]. There are two ways to
/// notify listeners:
///
/// 1. [`Event::notify()`] notifies a number of listeners.
/// 2. [`Event::notify_additional()`] notifies a number of previously unnotified
/// listeners.
///
/// If there are no active listeners at the time a notification is sent, it
/// simply gets lost.
///
...
/// will notify another active listener. Whether one *additional* listener will
/// be notified depends on what kind of notification was delivered.
///
/// Listeners are registered and notified in the first-in first-out fashion,
/// ensuring fairness.
pub struct Event {
    /// A pointer to heap-allocated inner state.
    ///
    /// This pointer is initially null and gets lazily initialized on first use.
    /// Semantically, it is an `Arsc<Inner>` so it's important to keep in
    /// mind that it contributes to the [`Arsc`]'s reference count.
...
impl Event {
    ...
    pub fn listen(&self) -> EventListener {
        ...
        let listener = EventListener {
            inner: unsafe { Arsc::clone(&ManuallyDrop::new(Arsc::from_raw(inner))) },
    ...
    ///
    /// The number is allowed to be zero or exceed the current number of
    /// listeners.
    ///
    /// In contrast to [`Event::notify_additional()`], this method only makes
    /// sure *at least* `n` listeners among the active ones are notified.
    ///
    ...
    /// listeners.
    ///
    /// In contrast to [`Event::notify()`], this method will notify `n`
    /// *additional* listeners that were previously unnotified.
    ///
    /// This method emits a `SeqCst` fence before notifying listeners.
    ///
    /// # Examples
    ...
    /// Notifies a number of active and still unnotified listeners without
    /// emitting a `SeqCst` fence.
    ///
    /// The number is allowed to be zero or exceed the current number of
    /// listeners.
    ///
    ...
    fn inner(&self) -> *const Inner {
        ...
        if inner.is_null() {
            ...
            let new = Arsc::into_raw(new) as *mut Inner;
...
fn full_fence() {
    if cfg!(all(
        ...
        // 2. `_.compare_exchange(_, _, SeqCst, SeqCst)`, which compiles into a `lock
        // cmpxchg` instruction.
        //
        // Both instructions have the effect of a full barrier, but empirical benchmarks
        // have shown that the second one is sometimes a bit faster.
        //
        // The ideal solution here would be to use inline assembly, but we're instead
        // creating a temporary atomic variable and compare-and-exchanging its
        // value. No sane compiler to x86 platforms is going to optimize this
        // away.
