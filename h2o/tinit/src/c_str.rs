...
use core::{
    ascii, borrow::Borrow, cmp::Ordering, fmt, fmt::Write, mem, num::NonZeroU8, ops, ptr, slice,
    str, str::Utf8Error,
};
...
/// a read-only pointer; if the C code writes to it, that causes
/// undefined behavior.
///
...
///
/// `&CStr` is to [`CString`] as <code>&[str]</code> is to [`String`]: the
/// former in each pair are borrowed references; the latter are owned
/// strings.
///
...
// When attribute privacy is implemented, `CStr` should be annotated as
// `#[repr(transparent)]`. Anyway, `CStr` representation and layout are
...
impl CString {
    ...
    /// obtained by calling [`CString::into_raw`]. Other usage (e.g., trying to
    /// take ownership of a string that was allocated by foreign code) is
    /// likely to lead to undefined behavior or allocator corruption.
    ///
    /// It should be noted that the length isn't just "recomputed," but that
    /// the recomputed length must match the original length from the
    /// [`CString::into_raw`] call. This means the
    /// [`CString::into_raw`]/`from_raw` methods should not be used when
    /// passing the string to C functions that can modify the string's
    /// length.
    ///
    ...
    /// reconstituted using [`CString::from_raw`] to be properly
    /// deallocated. Specifically, one should *not* use the standard C
    /// `free()` function to deallocate this string.
    ///
    ...
    /// Converts a <code>[Vec]<[u8]></code> to a [`CString`] without checking
    /// the invariants on the given [`Vec`].
    ///
    /// # Safety
    ///
...
impl<'a> From<Cow<'a, CStr>> for CString {
    /// Converts a `Cow<'a, CStr>` into a `CString`, by copying the contents if
    /// they are borrowed.
    #[inline]
...
impl From<Box<CStr>> for CString {
    /// Converts a <code>[Box]<[CStr]></code> into a [`CString`] without copying
    /// or allocating.
    #[inline]
...
impl From<CString> for Box<CStr> {
    /// Converts a [`CString`] into a <code>[Box]<[CStr]></code> without copying
    /// or allocating.
    #[inline]
...
impl From<CString> for Arc<CStr> {
    /// Converts a [`CString`] into an <code>[Arc]<[CStr]></code> by moving the
    /// [`CString`] data into a new [`Arc`] buffer.
    #[inline]
...
impl From<CString> for Rc<CStr> {
    /// Converts a [`CString`] into an <code>[Rc]<[CStr]></code> by moving the
    /// [`CString`] data into a new [`Arc`] buffer.
    #[inline]
...
impl CStr {
    ...
    pub unsafe fn from_ptr<'a>(ptr: *const c_char) -> &'a CStr {
        ...
        // from_raw_parts is safe because we know the length is at most `isize::MAX`,
        // meaning the call to `from_bytes_with_nul_unchecked` is correct.
        //
    ...
    /// after the `CString::new("Hello").expect("CString::new
    /// failed").as_ptr()` expression is evaluated.
    ...
    /// function will return the corresponding <code>&[str]</code> slice.
    /// Otherwise, it will return an error with details of where UTF-8
    /// validation failed.
    ///
    /// [str]: prim@str "str"
    ///
