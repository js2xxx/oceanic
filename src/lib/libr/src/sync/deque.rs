...
// Maximum number of tasks that can be stolen in `steal_batch()` and
// `steal_batch_and_pop()`.
const MAX_BATCH: usize = 32;
// If a buffer of at least this size is retired, thread-local garbage is flushed
// so that it gets deallocated as soon as possible.
const FLUSH_THRESHOLD_BYTES: usize = 1 << 10;

/// A buffer that holds tasks in a worker queue.
///
/// This is just a pointer to the buffer and its length - dropping an instance
/// of this struct will *not* deallocate the buffer.
struct Buffer<T> {
...
impl<T> Buffer<T> {
    ...
    unsafe fn at(&self, index: isize) -> *mut T {
        ...
        // We do all the loads at `MaybeUninit` because we might realize, after loading,
        // that we don't actually have the right to access this memory.
        self.ptr.offset(index & (self.cap - 1) as isize)
    }

    /// Writes `task` into the specified `index`.
    ///
    /// This method might be concurrently called with another `read` at the same
    /// index, which is technically speaking a data race and therefore UB.
    /// We should use an atomic store here, but that would be more expensive
    /// and difficult to implement generically for all types `T`.
    ...
    ///
    /// This method might be concurrently called with another `write` at the
    /// same index, which is technically speaking a data race and therefore
    /// UB. We should use an atomic load here, but that would be more
    /// expensive and difficult to implement generically for all types `T`.
...
///
/// 1. [Chase and Lev. Dynamic circular work-stealing deque. SPAA
/// 2005.][chase-lev] 2. [Le, Pop, Cohen, and Nardelli. Correct and efficient
/// work-stealing for weak memory models.    PPoPP 2013.][weak-mem]
/// 3. [Norris and Demsky. CDSchecker: checking concurrent data structures
/// written with C/C++    atomics. OOPSLA 2013.][checker]
///
...
/// A worker queue.
///
/// This is a FIFO or LIFO queue that is owned by a single thread, but other
/// threads may steal tasks from it. Task schedulers typically create a single
/// worker queue per thread.
///
/// # Examples
///
/// A FIFO worker:
...
impl<T> Worker<T> {
    ...
    /// Reserves enough capacity so that `reserve_cap` tasks can be pushed
...
impl<T> Stealer<T> {
    ...
    ///
    /// How many tasks exactly will be stolen is not specified. That said, this
    /// method will try to steal around half of the tasks in the queue, but
    /// also not more than some constant limit.
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// Steals a batch of tasks, pushes them into another worker, and pops a
    /// task from that worker.
    ///
    /// How many tasks exactly will be stolen is not specified. That said, this
    /// method will try to steal around half of the tasks in the queue, but
    /// also not more than some constant limit.
...
impl<T> Block<T> {
    ...
    /// Sets the `DESTROY` bit in slots starting from `start` and destroys the
    /// block.
...
/// An injector queue.
///
/// This is a FIFO queue that can be shared among multiple threads. Task
/// schedulers typically have a single injector queue, which is the entry point
/// for new tasks.
///
/// # Examples
///
...
impl<T> Injector<T> {
    ...
    pub fn steal_batch(&self, dest: &Worker<T>) -> Steal<()> {
        ...
        if new_head & HAS_NEXT == 0 {
            ...
            // If head and tail are not in the same block, set `HAS_NEXT` in head. Also,
            // calculate the right batch size to steal.
            if (head >> SHIFT) / LAP != (tail >> SHIFT) / LAP {
                new_head |= HAS_NEXT;
    ...
    /// Steals a batch of tasks, pushes them into a worker, and pops a task from
    /// that worker.
    ///
    /// How many tasks exactly will be stolen is not specified. That said, this
    /// method will try to steal around half of the tasks in the queue, but
...
impl<T> Steal<T> {
    ...
    ///
    /// Returns this steal result if it is `Success`. Otherwise, closure `f` is
    /// invoked and then:
    ///
    /// * If the second steal resulted in `Success`, it is returned.
    /// * If both steals were unsuccessful but any resulted in `Retry`, then
    ///   `Retry` is returned.
...
impl<T> FromIterator<Steal<T>> for Steal<T> {
    ...
    ///
    /// If no `Success` was found, but there was at least one `Retry`, then
    /// returns `Retry`. Otherwise, `Empty` is returned.
