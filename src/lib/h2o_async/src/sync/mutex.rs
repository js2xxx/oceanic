...
impl<T: ?Sized> Mutex<T> {
    ...
    pub async fn lock_arc(self: &Arc<Self>) -> MutexGuardArc<T> {
        self.clone().lock_arc_impl().await
    }
...
impl<T: ?Sized> Mutex<T> {
    async fn lock_arsc_impl(self: Arsc<Self>) -> MutexGuardArsc<T> {
        if let Some(guard) = self.try_lock_arsc() {
            return guard;
        }
        self.acquire_slow().await;
        MutexGuardArsc(self)
    ...
    /// Similar with [`lock_arc`], but with `Arsc`.
    #[inline]
    pub async fn lock_arsc(self: &Arsc<Self>) -> MutexGuardArsc<T> {
        self.clone().lock_arsc_impl().await
    }

    /// Similar with [`try_lock_arc`], but with `Arsc`.
    #[inline]
    pub fn try_lock_arsc(self: &Arsc<Self>) -> Option<MutexGuardArsc<T>> {
        if self
            .state
            .compare_exchange(0, 1, Ordering::Acquire, Ordering::Acquire)
            .is_ok()
        {
            Some(MutexGuardArsc(self.clone()))
        } else {
            None
        }
    }
...
/// An owned guard that releases the mutex when dropped.
pub struct MutexGuardArsc<T: ?Sized>(Arsc<Mutex<T>>);

unsafe impl<T: Send + ?Sized> Send for MutexGuardArsc<T> {}
unsafe impl<T: Sync + ?Sized> Sync for MutexGuardArsc<T> {}

impl<T: ?Sized> MutexGuardArsc<T> {
    /// Returns a reference to the mutex a guard came from.
    ///
    /// # Examples
    ///
    /// ```
    /// # futures_lite::future::block_on(async {
    /// use async_lock::{Mutex, MutexGuardArsc};
    /// use std::sync::Arc;
    ///
    /// let mutex = Arc::new(Mutex::new(10i32));
    /// let guard = mutex.lock_arc().await;
    /// dbg!(MutexGuardArsc::source(&guard));
    /// # })
    /// ```
    pub fn source(guard: &MutexGuardArsc<T>) -> &Arsc<Mutex<T>> {
        &guard.0
    }
}

impl<T: ?Sized> Drop for MutexGuardArsc<T> {
    fn drop(&mut self) {
        // Remove the last bit and notify a waiting lock operation.
        self.0.state.fetch_sub(1, Ordering::Release);
        self.0.lock_ops.notify(1);
...
impl<T: fmt::Debug + ?Sized> fmt::Debug for MutexGuardArsc<T> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        fmt::Debug::fmt(&**self, f)
    }
}

impl<T: fmt::Display + ?Sized> fmt::Display for MutexGuardArsc<T> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        (**self).fmt(f)
    }
}

impl<T: ?Sized> Deref for MutexGuardArsc<T> {
    type Target = T;
...
impl<T: ?Sized> DerefMut for MutexGuardArsc<T> {
    fn deref_mut(&mut self) -> &mut T {
        unsafe { &mut *self.0.data.get() }
    }
}
