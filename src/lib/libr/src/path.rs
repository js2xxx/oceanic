...
impl<'a> cmp::PartialEq for Components<'a> {
    ...
    fn eq(&self, other: &Components<'a>) -> bool {
        ...
        // either be covered by the `path` buffer or are only relevant for
        // `prefix_verbatim()`.
...
fn compare_components(mut left: Components<'_>, mut right: Components<'_>) -> cmp::Ordering {
    ...
    // - backtrack to find separator before mismatch to avoid ambiguous parsings of
    //   '.' or '..' characters
    // - if found update state to only do a component-wise comparison on the
    //   remainder, otherwise do it on the full path
    //
    // The fast path isn't taken for paths with a PrefixComponent to avoid
...
impl PathBuf {
    ...
    fn _set_extension(&mut self, extension: &OsStr) -> bool {
        ...
        let start = self.inner.bytes().as_ptr() as usize;
...
impl From<PathBuf> for Arc<Path> {
    /// Converts a [`PathBuf`] into an <code>[Arc]<[Path]></code> by moving the
    /// [`PathBuf`] data into a new [`Arc`] buffer.
    #[inline]
